                        ...
                        <div class="col-sm-12">
                            <div class="panel panel-white post">
                                ...
                                <div class="post-image">
                                    <h4><a href="{{ route('posts.show', $definition) }}">{{ $definition->title }}</a></h4>
                                    <img src=""><img src="{{ asset($definition->medium_image_path) }}" class="image" alt="image post">
                                </div>
                                <div class="post-description">
                                    <div class="stats">
                                        <div class="btn-group">
                                            <form action="{{ route('posts.like', $definition) }}" method="post">
                                                @csrf
                                                <button class="btn" type="submit"><i class="fa fa-thumbs-up icon"> {{ $definition->likes()->where('is_dislike', 0)->get()->count() }}</i></button>
                                            </form>
                                            <form action="{{ route('posts.dislike', $definition) }}" method="post">
                                                @csrf
                                                <button class="btn" type="submit"><i class="fa fa-thumbs-up icon"> {{ $definition->likes()->where('is_dislike', 1)->get()->count() }}</i></button>
                                            </form>
                                            <button class="btn"><a href="{{ route('posts.show', $definition) }}"><i class="fas fa-comment"></i> {{ $definition->comments()->where('approved', true)->count() }} {{ Str::plural('comment', $definition->comments()->count()) }}</i></a></button>
                                        </div>
                                    </div>
                                    <p class="mt-2">
                                        Tags:
